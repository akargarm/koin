...
public final class KoinToken_sol_KoinToken extends Contract {
    ...
    public List<TransferEventResponse> getTransferEvents(TransactionReceipt transactionReceipt) {
        ...
        for (EventValues eventValues : valueList) {
            ...
            typedResponse.recipient = (Address) eventValues.getNonIndexedValues().get(1);
    ...
    public Future<TransactionReceipt> approve(Address recipient, Uint256 amount) {
        Function function = new Function("approve", Arrays.<Type>asList(recipient, amount), Collections.<TypeReference<?>>emptyList());
    ...
    public Future<TransactionReceipt> transferFrom(Address sender, Address recipient, Uint256 amount) {
        Function function = new Function("transferFrom", Arrays.<Type>asList(sender, recipient, amount), Collections.<TypeReference<?>>emptyList());
        return executeTransactionAsync(function);
    }
    ...
    public Future<TransactionReceipt> _transfer(Address recipient, Uint256 amount) {
        Function function = new Function("_transfer", Arrays.<Type>asList(recipient, amount), Collections.<TypeReference<?>>emptyList());
        return executeTransactionAsync(function);
    }

    public Future<Uint256> balanceOf(Address user) {
    ...
    public Future<Uint256> allowance(Address sender, Address recipient) {
        Function function = new Function("allowance",
                Arrays.<Type>asList(sender, recipient),
                Arrays.<TypeReference<?>>asList(new TypeReference<Uint256>() {
                }));
        return executeCallSingleValueReturnAsync(function);
    }
    ...
    public static Future<KoinToken_sol_KoinToken> deploy(Web3j web3j, Credentials credentials, BigInteger gasPrice, BigInteger gasLimit, BigInteger initialWeiValue) {
        return deployAsync(KoinToken_sol_KoinToken.class, web3j, credentials, gasPrice, gasLimit, BINARY, "", initialWeiValue);
    }

    public static Future<KoinToken_sol_KoinToken> deploy(Web3j web3j, TransactionManager transactionManager, BigInteger gasPrice, BigInteger gasLimit, BigInteger initialWeiValue) {
        return deployAsync(KoinToken_sol_KoinToken.class, web3j, transactionManager, gasPrice, gasLimit, BINARY, "", initialWeiValue);
    }
